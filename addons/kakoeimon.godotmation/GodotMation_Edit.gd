...
func _process(delta):
	...
	var godotmation
	if drawing_area.playing and playing_godotmation:
		for i in range(drawing_area_nodes.get_child_count()):
			...
			if c1.type == 2 or c1.type == 9:
				c1.get_node("Number").text = str(playing_godotmation.get_GM_number(i))
			#elif c1.type == 0 and c2 and c2.get_type() <=1:
			#	c1.get_node("Label").text = c2.get_caption()
...
func _on_PlayButton_pressed():
	...
	godotmation.set_dict(_get_dict())
	
	#tools._build_from_dict(godotmation, _get_dict(), false, false)
	godotmation.setup()
	
	godotmation.start()
	playing_godotmation = godotmation
...
func set_author(value):
	author = value
	
func set_time_mode(value):
	time_mode = value
