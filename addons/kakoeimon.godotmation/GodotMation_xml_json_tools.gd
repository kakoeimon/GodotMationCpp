...
static func _build_from_dict(base, dict, in_editor = false, owner = false):
	if not dict.has("version"): return

	var nodes_container = base.get_node("DrawingArea/Nodes")
	var resource_pck = load("res://addons/kakoeimon.godotmation/editor_nodes/editor_resource.tscn")
	var state_pck = load("res://addons/kakoeimon.godotmation/editor_nodes/editor_state.tscn")
	var pool_pck = load("res://addons/kakoeimon.godotmation/editor_nodes/editor_pool.tscn")
	var gate_pck = load("res://addons/kakoeimon.godotmation/editor_nodes/editor_gate.tscn")
	var source_pck = load("res://addons/kakoeimon.godotmation/editor_nodes/editor_source.tscn")
	var drain_pck = load("res://addons/kakoeimon.godotmation/editor_nodes/editor_drain.tscn")
	var converter_pck = load("res://addons/kakoeimon.godotmation/editor_nodes/editor_converter.tscn")
	var delay_pck = load("res://addons/kakoeimon.godotmation/editor_nodes/editor_delay.tscn")
	var end_condition_pck = load("res://addons/kakoeimon.godotmation/editor_nodes/editor_end_condition.tscn")
	var function_call_pck = load("res://addons/kakoeimon.godotmation/editor_nodes/editor_function_call.tscn")

	base.set_version(dict.version)

	base.set_author(dict.author)
	base.set_time_mode(dict.time_mode)
	base.set_interval(dict.interval)
	base.set_actions(dict.actions)
	#base.distribution_mode = dict.distribution_mode
	#if dict.color_coding: print("Color Coding is not supported by GodotMation.")
	base.set_dice(dict.dice)
	...
	if dict.has("autostart"):
		base.set_autostart(dict.autostart)
	else:
		base.set_autostart(true)
		...
		if node.type >= 2:
			...
			match int(node.type):
				2:
					...
					child.set_type(2)
					#child.type = 2
					#child.set_starting_resources(node.starting_resources)
					child.set_number(node.starting_resources)
					child.set_capacity(node.capacity)
					child.set_emit_state_changed(node.emit_state_changed)
				3:
			...
			#print(node.caption)
			child.set_caption(node.caption)
			#child.caption_pos = node.caption_pos
			
			
			child.set_actions(node.actions)
			child.set_activation_mode(int(node.activation_mode))
			child.set_pull_mode(node.pull_mode)
			child.set_color(node.color)
			...
			if child.get_caption():
				child.name = child.get_caption()
...
#							child.points.append( Vector2(p[0], p[1]) )
			...
			child.set_color(node.color)
			child.set_end(node.end)
			child.set_start(node.start)
			child.set_caption(node.label)
			#child.thickness = node.thickness
			nodes_container.add_child(child)
			if in_editor:
